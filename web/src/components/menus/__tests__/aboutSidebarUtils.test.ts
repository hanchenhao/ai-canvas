...
describe("getAboutSidebarSections", () => {
  ...
  it("returns two sections", () => {
    ...
    expect(sections[1].category).toBe("System");
    expect(sections[2].category).toBe("Resources");
  });
  ...
  it("System section contains diagnostics", () => {
    const sections = getAboutSidebarSections();
    const system = sections[1];
